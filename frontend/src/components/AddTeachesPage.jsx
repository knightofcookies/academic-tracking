...
const AddTeachesPage = () => {
  ...
  useEffect(() => {
    ...
    adminServices.getAllCourses()
      .then(data => {
        setCourses(data);
        console.log(data);
      })
      .catch(error => console.error("Error fetching Courses:", error));
    adminServices.getAllInstructors()
      .then(data => {
        setInstructors(data);
        console.log(data);
      })
      .catch(error => console.error("Error fetching Instructors:", error));
    adminServices.getAllSessions()
      .then(data => {
        setSessions(data);
        console.log(data);
      })
  ...
  const handleSubmit = (event) => {
    ...
    const instructorId = instructors.find(instructor => instructor.name === selectedInstructor)?.id;
    const courseId = courses.find(course => course.code === selectedCourse)?.id;
    const sessionId = sessions.find(session => (session.season == sessionDetails[0].toLowerCase() && session.start_year == sessionDetails[1]))?.id;
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ width: '100%' }}>
                <FormControl style={{ marginTop: 16, minWidth: 120 }} fullWidth>
                  <InputLabel id="instructor">Instructors</InputLabel>
                  <Select
                    labelId="instructor"
                  ...
                  >
                    ...
                    {instructors.map((instructor) => (
                        <MenuItem key={instructor.id} value={instructor.name}>{instructor.name}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
                <FormControl style={{ marginTop: 16, minWidth: 120 }} fullWidth>
                ...
                <FormControl style={{ marginTop: 16, minWidth: 120 }} fullWidth>
                  ...
                  >
                    ...
                    {sessions.map((session) => (
                        <MenuItem key={session.id} value={`${session.season.toUpperCase()} ${session.start_year}`}>
